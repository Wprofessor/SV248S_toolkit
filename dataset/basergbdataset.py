...
# import __file__ as basepath

# _basepath = os.path.dirname(basepath)


def initial_gt_file(gt_path: str, seqs: list, v_name: str, bbox_trans):
    res = {}
    for seq_name in seqs:
        serial_v = load_text(os.path.join(gt_path, seq_name, v_name))
        seq_serial = {'visible': serial_process(bbox_trans, serial_v)}
...
class BaseRGBDataet:
    ...
    def __init__(self, gt_path: str, seqs: list, bbox_type: str, v_name=None) -> None:
        """
        [in] gt_path - str
        ...
        if v_name != None:
            self.seqs_gt = initial_gt_file(self.gt_path, seqs, v_name, self.bbox_transfun)  # ground truth
        else:
